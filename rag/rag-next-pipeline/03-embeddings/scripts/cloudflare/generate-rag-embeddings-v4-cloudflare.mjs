 ...
 *   Use --validate-only for offline input validation.
 *
 ...
 * INPUT
 * -----
 *   rag-next-pipeline/02-retrieval-documents/output/documents.jsonl
 *
 * OUTPUT (parallel to the validated Nomic artifacts)
 * -------------------------------------------------
 *   rag-next-pipeline/03-embeddings/output/embeddings-cloudflare-v1/embeddings.npy
 *   rag-next-pipeline/03-embeddings/output/embeddings-cloudflare-v1/embedding-records.jsonl
 *   rag-next-pipeline/03-embeddings/output/embeddings-cloudflare-v1/embedding-manifest.json
 *   rag-next-pipeline/03-embeddings/output/embeddings-cloudflare-v1/embedding-validation-report.txt
 *
 * RESUMABLE CHECKPOINTS
 * ---------------------
 *   rag-next-pipeline/03-embeddings/.embedding-cloudflare-v1-checkpoint/
 *
 * MODEL CONTRACT
 * --------------
...
function findRagRoot(startDir) {
  ...
  while (true) {
    const scripts = join(current, "01-corpus", "scripts");
    const corpus = join(current, "01-corpus", "output");
    if (current.split(/[\\/]/).pop() === "rag-next-pipeline" && existsSync(scripts) && existsSync(corpus)) {
      return current;
    }
  ...
  throw new Error("Could not locate the enclosing rag-next-pipeline/ root. Expected this script under rag-next-pipeline/.");
}
...
const CHECKPOINT_DIR = join(RAG_ROOT, "03-embeddings", ".embedding-cloudflare-v1-checkpoint");
...
async function runCloudflareEmbedding({ accountId, authHeaders, payload, expectedCount, requestLabel, counters }) {
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt += 1) {
    ...
    try {
      ...
      response = await (runtimeDependencies.fetch || globalThis.fetch)(url, {
        method: "POST",
        headers: {
          ...authHeaders,
...
// Cache identity is independent of corpus order, IDs, and logical batch size.
function embeddingContract() {
  return { provider: PROVIDER, model: MODEL, dimensions: EMBEDDING_DIMENSIONS,
    dtype: DTYPE, postprocess: POSTPROCESS, similarity: SIMILARITY,
    document_input_mode: DOCUMENT_INPUT_MODE, runtime_query_input_mode: QUERY_INPUT_MODE,
    runtime_query_instruction: QUERY_INSTRUCTION, field_embedded: "embedding_text" };
}
function textHash(document) { return sha256Text(String(document.embedding_text)); }
function cachePath(document) {
  return join(CACHE_DIR, sha256Text(stableStringify(embeddingContract())), `${textHash(document)}.json`);
}
function validateUnitVector(vector, label) {
  if (!Array.isArray(vector) || vector.some(v => typeof v !== "number" || !Number.isFinite(v))) throw new PipelineError(`${label}: invalid numeric vector`);
  const { vector: normalized } = l2Normalize(vector);
  if (vector.some((v, i) => Math.abs(v - normalized[i]) > UNIT_NORM_TOLERANCE)) {
    throw new PipelineError(`${label}: vector is not L2 normalized`);
  }
}
async function loadCachedVector(document) {
  const path = cachePath(document);
  if (!existsSync(path)) return null;
  const entry = JSON.parse(await fs.readFile(path, "utf8"));
  if (entry.cache_schema_version !== "1.0.0"
      || stableStringify(entry.contract) !== stableStringify(embeddingContract())
      || entry.embedding_text_sha256 !== textHash(document)
      || entry.vector_sha256 !== sha256Text(stableStringify(entry.vector))) {
    throw new PipelineError(`Incompatible or corrupt vector cache: ${rel(path)}`);
  }
  validateUnitVector(entry.vector, rel(path));
  return entry.vector;
}
async function saveCachedVector(document, vector) {
  validateUnitVector(vector, document.document_id);
  const existing = await loadCachedVector(document);
  if (existing) {
    if (stableStringify(existing) !== stableStringify(vector)) {
      throw new PipelineError(`Cache/checkpoint vector conflict: ${document.document_id}`);
    }
    return;
  }
  const path = cachePath(document);
  await writeJson(`${path}.tmp`, { cache_schema_version: "1.0.0", contract: embeddingContract(),
    embedding_text_sha256: textHash(document), vector_sha256: sha256Text(stableStringify(vector)), vector });
  renameSync(`${path}.tmp`, path);
}
async function inspectCheckpoint(identity, documents) {
  if (!existsSync(CHECKPOINT_DIR)) return;
  const path = join(CHECKPOINT_DIR, "identity.json");
  if (!existsSync(path) || stableStringify(JSON.parse(await fs.readFile(path, "utf8"))) !== stableStringify(identity)) {
    throw new PipelineError("Existing checkpoint identity does not match this embedding build; preserve and inspect it before starting another corpus/contract.");
  }
  ...
  for (let start = 0; start < documents.length; start += LOGICAL_BATCH_SIZE) {
    ...
    const path = batchCheckpointPath(start, end);
    allowed.add(path.split(/[\\/]/).pop());
    if (existsSync(path)) validateCheckpointBatch(JSON.parse(await fs.readFile(path, "utf8")), documents, start, end);
  }
  for (const name of await fs.readdir(CHECKPOINT_DIR)) {
    if (!allowed.has(name) && !name.endsWith(".tmp")) throw new PipelineError(`Unexpected checkpoint file: ${name}`);
  }
}
async function missingDocumentCount(documents) {
  let missing = 0;
  for (let start = 0; start < documents.length; start += LOGICAL_BATCH_SIZE) {
    const end = Math.min(documents.length, start + LOGICAL_BATCH_SIZE);
    const path = batchCheckpointPath(start, end);
    const checkpoint = existsSync(path) ? validateCheckpointBatch(JSON.parse(await fs.readFile(path, "utf8")), documents, start, end) : null;
    for (let i = start; i < end; i++) {
      const cached = await loadCachedVector(documents[i]);
      if (checkpoint && cached && stableStringify(cached) !== stableStringify(checkpoint.vectors[i - start])) {
        throw new PipelineError(`Cache/checkpoint vector conflict: ${documents[i].document_id}`);
      }
      if (!checkpoint && !cached) missing++;
    }
  }
  return missing;
}
async function reuseCompletedBuild(inputSha256, documents) {
  ...
  for (const [key, value] of Object.entries(embeddingContract())) {
    if (manifest.embedding?.[key] !== value) throw new PipelineError(`Existing output embedding contract mismatch: ${key}`);
  }
  for (const name of ["embeddings.npy", "embedding-records.jsonl"]) {
    if (!manifest.artifacts?.[name]?.sha256 || await sha256File(join(OUTPUT_DIR, name)) !== manifest.artifacts[name].sha256) {
      throw new PipelineError(`Existing output artifact hash mismatch: ${name}`);
    }
  }
  const count = manifest.input?.document_count;
  if (!Number.isInteger(count) || count < 1) throw new PipelineError("Existing output has invalid document count");
  await validateNpy(join(OUTPUT_DIR, "embeddings.npy"), count);
  const records = loadJsonlSync(join(OUTPUT_DIR, "embedding-records.jsonl"));
  if (records.length !== count || manifest.artifacts["embedding-records.jsonl"].records !== count) throw new PipelineError("Existing output record count mismatch");
  if (manifest.input.sha256 !== inputSha256) return false; // Valid earlier corpus; persistent cache handles reuse.
...
async function embedWithCheckpoints(documents, accountId, auth, counters) {
  let newBatches = 0, resumedBatches = 0, cachedDocuments = 0, newDocuments = 0;
  let rawNormMin = Infinity, rawNormMax = 0;
  const totalBatches = Math.ceil(documents.length / LOGICAL_BATCH_SIZE);
  for (let start = 0; start < documents.length; start += LOGICAL_BATCH_SIZE) {
    const end = Math.min(documents.length, start + LOGICAL_BATCH_SIZE);
    const path = batchCheckpointPath(start, end);
    if (existsSync(path)) {
      const batch = validateCheckpointBatch(JSON.parse(await fs.readFile(path, "utf8")), documents, start, end);
      for (let i = start; i < end; i++) await saveCachedVector(documents[i], batch.vectors[i - start]);
      for (const norm of batch.raw_norms || []) { if (Number.isFinite(norm)) { rawNormMin = Math.min(rawNormMin, norm); rawNormMax = Math.max(rawNormMax, norm); } }
      resumedBatches++;
      continue;
    }
    const vectors = [], rawNorms = [], missing = new Map();
    for (let i = start; i < end; i++) {
      const vector = await loadCachedVector(documents[i]);
      vectors.push(vector);
      rawNorms.push(null); // Original model norm is unavailable for a cached normalized vector.
      if (vector) cachedDocuments++;
      else if (!missing.has(textHash(documents[i]))) missing.set(textHash(documents[i]), documents[i]);
    }
    if (missing.size) {
      const fresh = [...missing.values()];
      const response = await runCloudflareEmbedding({ accountId, authHeaders: auth.headers,
        payload: { documents: fresh.map(d => String(d.embedding_text)) }, expectedCount: fresh.length,
        requestLabel: `documents ${start}-${end - 1}`, counters });
      for (let i = 0; i < fresh.length; i++) {
        await saveCachedVector(fresh[i], response.vectors[i]);
        missing.set(textHash(fresh[i]), { vector: response.vectors[i], rawNorm: response.rawNorms[i] });
      }
      for (let i = start; i < end; i++) if (!vectors[i - start]) {
        const item = missing.get(textHash(documents[i]));
        vectors[i - start] = item.vector; rawNorms[i - start] = item.rawNorm;
      }
      newBatches++; newDocuments += fresh.length;
    }
    for (const norm of rawNorms) { if (Number.isFinite(norm)) { rawNormMin = Math.min(rawNormMin, norm); rawNormMax = Math.max(rawNormMax, norm); } }
    await saveCheckpointBatch(path, { checkpoint_batch_schema_version: "1.0.0", created_at_utc: utcNow(),
      start, end_exclusive: end, document_ids: documents.slice(start, end).map(d => d.document_id),
      raw_norms: rawNorms, vectors });
  }
  return { logical_batch_count: totalBatches, new_logical_batches: newBatches,
    resumed_logical_batches: resumedBatches, cached_document_count: cachedDocuments,
    new_document_embeddings: newDocuments, raw_model_l2_norm_min: Number.isFinite(rawNormMin) ? rawNormMin : null,
...
async function writeTempOutputs({ documents, inputSha256, inputStats, runStats, smokeStats, authSource, accountSource, counters }) {
  ...
  const manifest = {
    ...
    next_pipeline_input: {
      vectors: "03-embeddings/output/embeddings-cloudflare-v1/embeddings.npy",
      records: "03-embeddings/output/embeddings-cloudflare-v1/embedding-records.jsonl",
      configuration: "03-embeddings/output/embeddings-cloudflare-v1/embedding-manifest.json",
    },
  };
  ...
  const report = [
    ...
    "",
    "OUTPUT",
    "  03-embeddings/output/embeddings-cloudflare-v1/embeddings.npy",
    "  03-embeddings/output/embeddings-cloudflare-v1/embedding-records.jsonl",
    "  03-embeddings/output/embeddings-cloudflare-v1/embedding-manifest.json",
    "  03-embeddings/output/embeddings-cloudflare-v1/embedding-validation-report.txt",
    "",
    "PRESERVATION",
    "  03-embeddings/output/embeddings-v2/ modified: NO",
...
function publishTempOutput() {
  const backup = join(RAG_ROOT, "03-embeddings", ".embeddings-cloudflare-v1.previous");
...
function printHeader() {
  ...
  console.log("  Existing 03-embeddings/output/embeddings-v2/ remains untouched.");
  console.log("  New output: 03-embeddings/output/embeddings-cloudflare-v1/");
  console.log();
}

async function executeMain({ argv = process.argv.slice(2), dependencies = {} } = {}) {
  runtimeDependencies = dependencies;
  printHeader();
  if (argv.length !== 1 || !["--validate-only", "--generate"].includes(argv[0])) {
    throw new PipelineError("Usage: --validate-only or --generate (exactly one explicit mode)");
  }
  const manifestPath = join(dirname(INPUT_PATH), "document-manifest.json");
  const manifest = JSON.parse(readFileSync(manifestPath, "utf8"));
  const manifestSha256 = await sha256File(manifestPath);
  if (String(manifest.schema_version).split(".")[0] !== EXPECTED_DOCUMENT_SCHEMA_MAJOR) {
    throw new PipelineError("Unsupported Stage 02 manifest schema");
  }
  EXPECTED_DOCUMENT_COUNT = manifest.statistics?.documents;
  EXPECTED_REPOSITORY_TOTAL = manifest.statistics?.repository_total;
  if (![EXPECTED_DOCUMENT_COUNT, EXPECTED_REPOSITORY_TOTAL].every(n => Number.isInteger(n) && n > 0)
      || manifest.statistics.repositories_covered !== EXPECTED_REPOSITORY_TOTAL) {
    throw new PipelineError("Invalid Stage 02 manifest counts/coverage");
  }
  const stage1Path = join(RAG_CORPUS_DIR, "manifest.json");
  if (await sha256File(stage1Path) !== manifest.input?.manifest_sha256
      || await sha256File(join(RAG_CORPUS_DIR, "repositories.jsonl")) !== manifest.input?.sha256) {
    throw new PipelineError("Stage 02 upstream manifest/input SHA-256 mismatch");
  }
  const stage1 = JSON.parse(readFileSync(stage1Path, "utf8"));
  if (stage1.stats.repository_count !== EXPECTED_REPOSITORY_TOTAL || manifest.input.repository_count !== EXPECTED_REPOSITORY_TOTAL) {
    throw new PipelineError("Stage 01/02 repository count mismatch");
  }
  for (const [name, info] of Object.entries(manifest.artifacts || {})) {
    if (name.includes("/") || name.includes("\\") || await sha256File(join(dirname(INPUT_PATH), name)) !== info.sha256) {
      throw new PipelineError(`Stage 02 artifact SHA-256 mismatch: ${name}`);
    }
  }
  if (!manifest.artifacts?.["documents.jsonl"]?.sha256) throw new PipelineError("Missing documents hash");
  ...
  await writeJson(join(RAG_ROOT, "03-embeddings", "validation", "local-validation.json"), {
    status: "PASS", input: inputStats, upstream_manifest_sha256: manifestSha256,
    documents_sha256: inputSha256, remote_calls: 0,
    boundary: "Stopped before credentials, Workers AI, Vectorize, D1, or deployment"
  });
  console.log("LOCAL VALIDATION PASS; stopped before Cloudflare authentication/embedding.");
  return { mode: "validate-only", remote_calls: 0, document_count: documents.length };
  }
  ...
  const missing = await missingDocumentCount(documents);
  await prepareCheckpoint(identity);

  console.log("[2/10] Resolve existing Cloudflare/Wrangler authentication ... ", "");
  const account = missing ? await (dependencies.resolveAccount || resolveCloudflareAccountId)() : { accountId: null, source: "local reuse" };
  const auth = missing ? await (dependencies.resolveAuth || resolveCloudflareAuth)() : { headers: {}, source: "local reuse" };
  ...
  const smokeStats = missing ? await runSmokeTests(account.accountId, auth, counters) : { document_mode: "SKIPPED: local reuse", query_mode: "SKIPPED: local reuse", document_shape: null, query_shape: null };
  ...
  console.log("      SUCCESS (compatibility already checked before authentication)");
  ...
  console.log("[7/10] Assemble manifest-derived rows x 1024 float32 NPY and evidence records ... ", "");
  ...
  console.log();
  console.log("OUTPUT");
  console.log("  03-embeddings/output/embeddings-cloudflare-v1/embeddings.npy");
  console.log("  03-embeddings/output/embeddings-cloudflare-v1/embedding-records.jsonl");
  console.log("  03-embeddings/output/embeddings-cloudflare-v1/embedding-manifest.json");
  console.log("  03-embeddings/output/embeddings-cloudflare-v1/embedding-validation-report.txt");
  console.log();
  console.log("NEXT STAGE");
  console.log("  Evaluate this generation before any separate Vectorize publication.");
  return { mode: "generate", remote_calls: counters.http_attempts, ...runStats };
}

// Imported tests inject authentication/fetch. CLI defaults retain real behavior.
let executionActive = false;
export async function main(options = {}) {
  if (executionActive) throw new PipelineError("Stage 03 is already running in this module");
  executionActive = true;
  try { return await executeMain(options); }
  finally { executionActive = false; runtimeDependencies = {}; }
}

if (process.argv[1] && resolve(process.argv[1]) === SCRIPT_PATH) main().catch((error) => {
  console.error();
  ...
  if (existsSync(CHECKPOINT_DIR)) {
    ...
    console.error("Rerun the same --generate command to resume after fixing the issue/quota reset.");
  }
  rmSync(TEMP_OUTPUT_DIR, { recursive: true, force: true });
  process.exitCode = 1;
});
