...
OUTPUT
------
    03-embeddings/output/embeddings-v2/embeddings.npy
    03-embeddings/output/embeddings-v2/embedding-records.jsonl
    03-embeddings/output/embeddings-v2/embedding-manifest.json
    03-embeddings/output/embeddings-v2/embedding-validation-report.txt

A resumable private checkpoint directory may exist while the step is running:

    03-embeddings/.embedding-v2-checkpoint/
...
def find_rag_root(start: Path) -> Path:
    for candidate in (start, *start.parents):
        if candidate.name == "rag-next-pipeline" and (candidate / "01-corpus" / "scripts").is_dir() and (candidate / "01-corpus" / "output").is_dir():
            return candidate
    raise RuntimeError(
...
INPUT_PATH = RAG_ROOT / "02-retrieval-documents" / "output" / "documents.jsonl"
OUTPUT_DIR = RAG_ROOT / "03-embeddings" / "output" / "embeddings-v2"
TEMP_OUTPUT_DIR = RAG_ROOT / "03-embeddings" / ".embeddings-v2.tmp"
CHECKPOINT_DIR = RAG_ROOT / "03-embeddings" / ".embedding-v2-checkpoint"
...
def write_temp_outputs(
    ...
    manifest = {
        ...
        "next_pipeline_input": {
            "vectors": "03-embeddings/output/embeddings-v2/embeddings.npy",
            "records": "03-embeddings/output/embeddings-v2/embedding-records.jsonl",
            "configuration": "03-embeddings/output/embeddings-v2/embedding-manifest.json",
        },
    }
...
def publish_temp_output() -> None:
    backup_dir = RAG_ROOT / "03-embeddings" / ".embeddings-v2.previous"
...
def print_header() -> None:
    ...
    print()
    print("INPUT")
    print("  02-retrieval-documents/output/documents.jsonl")
    ...
    print()
    print("OUTPUT")
    print("  03-embeddings/output/embeddings-v2/embeddings.npy")
    print("  03-embeddings/output/embeddings-v2/embedding-records.jsonl")
    print("  03-embeddings/output/embeddings-v2/embedding-manifest.json")
    print("  03-embeddings/output/embeddings-v2/embedding-validation-report.txt")
    print()
...
def main() -> int:
    ...
    try:
        ...
        print("Existing validated 03-embeddings/output/embeddings-v2/ output, if any, was not replaced")
