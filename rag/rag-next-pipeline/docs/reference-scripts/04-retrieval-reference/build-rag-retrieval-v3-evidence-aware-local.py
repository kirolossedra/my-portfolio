...
def find_rag_root(start: Path) -> Path:
    for candidate in (start, *start.parents):
        if candidate.name == "rag-next-pipeline" and (candidate / "01-corpus" / "scripts").is_dir() and (candidate / "01-corpus" / "output").is_dir():
            return candidate
    raise RuntimeError("Could not locate the enclosing rag-next-pipeline/ root. Expected this script to live under rag-next-pipeline/.")

RAG_ROOT = find_rag_root(SCRIPT_DIR)
BASE_DIR = RAG_ROOT
RAG_DIR = RAG_ROOT / "01-corpus" / "output"
EMBEDDINGS_DIR = RAG_ROOT / "03-embeddings" / "output" / "embeddings-v2"
...
OUTPUT_DIR = RAG_ROOT / "docs" / "reference-output" / "retrieval-v3"
TEMP_OUTPUT_DIR = RAG_ROOT / "docs" / "reference-output" / ".retrieval-v3.tmp"
