...
def find_rag_root(start: Path) -> Path:
    for candidate in (start, *start.parents):
        if candidate.name == "rag-next-pipeline" and (candidate / "01-corpus" / "scripts").is_dir() and (candidate / "01-corpus" / "output").is_dir():
            return candidate
    raise RuntimeError("Could not locate the enclosing rag-next-pipeline/ root. Expected this script under rag-next-pipeline/.")

PROJECT_ROOT = find_rag_root(SCRIPT_DIR)
EMBEDDINGS_PATH = PROJECT_ROOT / "03-embeddings" / "output" / "embeddings-v2" / "embeddings.npy"
RECORDS_PATH = PROJECT_ROOT / "03-embeddings" / "output" / "embeddings-v2" / "embedding-records.jsonl"
MANIFEST_PATH = PROJECT_ROOT / "03-embeddings" / "output" / "embeddings-v2" / "embedding-manifest.json"
DOCUMENTS_PATH = PROJECT_ROOT / "02-retrieval-documents" / "output" / "documents.jsonl"
