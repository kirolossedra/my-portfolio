...
directory. It locates the enclosing ``rag-next-pipeline/`` root from its own file location,
discovers only ``00-source/portfolio-rag/repositories/repo-*/README.md``,
and always publishes the canonical corpus to the isolated ``01-corpus/output/``.
...
def find_rag_root(start: Path) -> Path:
    for candidate in (start, *start.parents):
        if (
            candidate.name == "rag-next-pipeline"
            and (candidate / "01-corpus" / "scripts").is_dir()
            and (candidate / "00-source" / "portfolio-rag").is_dir()
        ):
            return candidate
    raise RuntimeError(
        "Could not locate the enclosing rag-next-pipeline/ root. Expected this script to live under rag-next-pipeline/."
    )


RAG_ROOT = find_rag_root(SCRIPT_DIR)
PROJECT_ROOT = RAG_ROOT / "00-source" / "portfolio-rag"
BASE_DIR = RAG_ROOT
OUTPUT_DIR = RAG_ROOT / "01-corpus" / "output"
TEMP_OUTPUT_DIR = RAG_ROOT / "01-corpus" / ".output.tmp"

# Only repository README files are inputs. Batch reports, continuation files,
# README.md, ledgers, generated output and archived implementations are excluded.
INPUT_GLOB = "repositories/repo-*/README.md"
...
REPO_HEADER_RE = re.compile(
    r"^#\s+Repository\s+(?P<index>\d+)\s+—\s+(?P<title>.+?)\s*$",
    re.MULTILINE,
)
...
def load_source_manifest() -> dict[str, Any]:
    ...
    count = manifest.get("processedRepositories")
    if type(count) is not int or count < 1:
        raise RuntimeError("Manifest processedRepositories must be a positive integer")
    if "repositoryLineCounts" in manifest:
        if set(manifest["repositoryLineCounts"]) != {str(i) for i in range(1, count + 1)}:
            raise RuntimeError("Manifest processed index inventory is not contiguous")
    return manifest


def discover_input_files() -> list[InputFile]:
    count = load_source_manifest()["processedRepositories"]
    paths = sorted(PROJECT_ROOT.glob(INPUT_GLOB))
    if len(paths) != count:
        raise RuntimeError(f"Manifest processed-count mismatch: expected {count}, discovered {len(paths)}")
    resolved = [p.resolve() for p in paths]
    if len(set(resolved)) != len(paths):
        raise RuntimeError("Duplicate repository paths")
    for path in paths:
        if not path.is_file() or not path.resolve().is_relative_to(PROJECT_ROOT.resolve()):
            raise RuntimeError(f"Invalid source path: {path}")
    return [InputFile(path=p, priority=10) for p in paths]
...
def parse_repository_blocks(input_file: InputFile) -> list[RepoBlock]:
    # Preserve exact decoded bytes, including trailing whitespace and CRLF.
    text = input_file.path.read_bytes().decode("utf-8")
    headers = []
    fence = None
    for line in text.splitlines():
        ...
        if marker:
            ...
            continue
        if fence is None and re.match(r"^#\s+", line): headers.append(line)
    if not headers or text.splitlines()[0] != headers[0]:
        raise RuntimeError(f"Missing leading repository H1/index in {input_file.path}")
    match = REPO_HEADER_RE.fullmatch(headers[0])
    if match:
        if sum(bool(REPO_HEADER_RE.fullmatch(h)) for h in headers) != 1:
            raise RuntimeError("Multiple repository analyses in one README")
        index, title = int(match.group("index")), match.group("title").strip()
    else:
        if headers[0].startswith("# Repository "):
            raise RuntimeError("Malformed repository H1/index")
        identity = re.search(r"^## Repository Identity\s*\n(.*?)(?=^#{1,2} |\Z)", text, re.M | re.S)
        identity_text = identity.group(1).replace("**", "") if identity else ""
        indexes = re.findall(r"^- (?:Repository(?: index)?|Corpus index):\s*(\d+)(?:\s*/\s*\d+)?\s*$", identity_text, re.M | re.I)
        if len(indexes) != 1:
            raise RuntimeError("Missing or ambiguous repository identity index")
        index, title = int(indexes[0]), headers[0][2:].strip()
    if any(h != "# Project Tags" for h in headers[1:]):
        raise RuntimeError("Unexpected extra H1; possible multiple repository analyses")
    folder = re.fullmatch(r"repo-(\d+)-(.+)", input_file.path.parent.name)
    if not folder or int(folder.group(1)) != index or index < 1 or folder.group(2) != title:
        raise RuntimeError(f"Repository index/name/path mismatch in {input_file.path}")
    return [RepoBlock(index=index, total=load_source_manifest()["processedRepositories"],
        header_title=title, repo_name=title,
        source_file=input_file.path.relative_to(PROJECT_ROOT).as_posix(),
        source_line_start=1, source_line_end=len(text.splitlines()), raw=text,
...
def select_repository_blocks(input_files: list[InputFile]) -> tuple[dict[int, RepoBlock], list[str]]:
    ...
    for input_file in input_files:
        ...
        if path in paths: raise RuntimeError("Duplicate repository paths")
        paths.add(path)
        block, = parse_repository_blocks(input_file)
        if block.index in selected: raise RuntimeError(f"Duplicate repository index {block.index}")
...
def extract_retrieval_tags(raw: str, sections: list[dict[str, Any]], metadata: dict[str, str]) -> list[str]:
    ...
    candidates.extend(re.findall(r"^# Project Tags\s*\n(.*?)(?=^# |\Z)", raw, re.M | re.S))
    tags: list[str] = []
...
def build_outputs(input_files: list[InputFile], blocks: dict[int, RepoBlock], duplicate_notes: list[str], validation_warnings: list[str], expected_total: int) -> dict[str, Any]:
    ...
    input_manifest = [{
        "file": item.path.relative_to(PROJECT_ROOT).as_posix(), "size_bytes": item.path.stat().st_size,
    ...
    manifest = {
        ...
        "source_manifest": {"path": "corpus-manifest.json", "sha256": sha256_file(PROJECT_ROOT / "corpus-manifest.json"), "processedRepositories": expected_total},
        "repository_indexes": sorted(blocks),
        "artifacts": {p.name: {"sha256": sha256_file(p), "bytes": p.stat().st_size} for p in [TEMP_OUTPUT_DIR / "repositories.jsonl", TEMP_OUTPUT_DIR / "repository-catalog.json"]},
    }
    ...
    report_lines = [
        ...
        f"Repository coverage: {len(records)}/{expected_total}", f"Input repository README files: {len(input_files)}",
...
def main() -> int:
    if len(sys.argv) != 1:
        print(f"ERROR: {SCRIPT_NAME} intentionally accepts no arguments.\nRun:\n  python rag-next-pipeline/01-corpus/scripts/{SCRIPT_NAME}", file=sys.stderr)
        return 2
    ...
    try:
        ...
        print(f"Found {len(input_files)} repository README file(s):")
        for item in input_files:
        ...
        print("\nNext pipeline stage should consume 01-corpus/output/repositories.jsonl")
