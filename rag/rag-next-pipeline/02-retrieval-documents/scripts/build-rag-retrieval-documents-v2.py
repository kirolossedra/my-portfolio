...
Run from any working directory:
    ...
    python rag-next-pipeline/02-retrieval-documents/scripts/build-rag-retrieval-documents-v2.py
...
INPUT
-----
    01-corpus/output/repositories.jsonl

OUTPUT
------
    02-retrieval-documents/output/documents.jsonl
    02-retrieval-documents/output/document-catalog.json
    02-retrieval-documents/output/document-manifest.json
    02-retrieval-documents/output/document-validation-report.txt
    02-retrieval-documents/output/excluded-source-units.jsonl
    02-retrieval-documents/output/by-repository/repo-XXX.documents.jsonl
"""
...
BASE_DIR = RAG_ROOT
RAG_DIR = RAG_ROOT / "01-corpus" / "output"
INPUT_PATH = RAG_DIR / "repositories.jsonl"
OUTPUT_DIR = RAG_ROOT / "02-retrieval-documents" / "output"
TEMP_OUTPUT_DIR = RAG_ROOT / "02-retrieval-documents" / ".output.tmp"
BY_REPO_DIRNAME = "by-repository"

SCHEMA_VERSION = "2.0.0"
EXPECTED_INPUT_SCHEMA_MAJOR = "1"
sys.path.insert(0, str(RAG_ROOT / "01-corpus" / "scripts"))
from retrieval_clean import clean_markdown
from manifest_contract import validate_stage1
EXPECTED_REPOSITORY_TOTAL = None
...
def validate_input_records(records: list[dict[str, Any]]) -> int:
    global EXPECTED_REPOSITORY_TOTAL, UPSTREAM_MANIFEST_SHA256
    upstream = validate_stage1(RAG_ROOT)
    EXPECTED_REPOSITORY_TOTAL = upstream["stats"]["repository_count"]
    UPSTREAM_MANIFEST_SHA256 = sha256_file(RAG_DIR / "manifest.json")
...
def build_fragments_for_repo(record: dict[str, Any], blocks: list[SourceBlock], fp_counts: Counter[str], title_counts: Counter[str], template_threshold: int, generic_title_threshold: int) -> tuple[list[EvidenceFragment], list[dict[str, Any]], dict[str, int]]:
    ...
    for block in blocks:
        ...
        if retrieval_class == "limitation" or polarity in {"negative", "mixed"}:
            suppress = False
        if suppress:
            ...
            continue
        if retrieval_class != "limitation" and polarity not in {"negative", "mixed"} and block.word_count < 16 and generic_title and concrete_hits == 0 and not block.related_skill_ratings:
...
def assemble_authoritative_text(fragments: list[EvidenceFragment]) -> str:
    ...
    for fragment in fragments:
        ...
        if path != last_path:
            parts.append(clean_markdown(f"Section: {' > '.join(path)}"))
            last_path = path
        parts.append(clean_markdown(fragment.block.text))
...
def _make_document(record: dict[str, Any], fragments: list[EvidenceFragment], semantic_area: str) -> dict[str, Any] | None:
    ...
    roots = unique_preserving(root_heading(f.block.section_path) for f in fragments)
    wc = word_count(text)
    if wc < 26 and concrete_hits == 0 and specificity < 0.40 and retrieval_class != "limitation" and polarity not in {"negative", "mixed"}: return None
...
def validation_report_text(stats: dict[str, Any], build_stats: dict[str, Any], manifest: dict[str, Any]) -> str:
    ...
    lines.extend(["", "INTEGRITY", "  Step 1 repository SHA-256 values verified: YES", "  Manifest-derived repository coverage verified: YES", "  Source-fragment text hashes verified: YES", "  Source-fragment provenance bounds verified: YES", "  Duplicate document IDs: 0", "  Step 1 canonical corpus modified: NO", "", "NEXT PIPELINE INPUT", "  02-retrieval-documents/output/documents.jsonl", "", "STEP 3 RULE", "  Embed `embedding_text`; retain `text` + `source_fragments` for evidence/citations.", "  Do NOT reuse old 01-corpus/output/chunks/chunks.jsonl.", "", f"Manifest SHA-256 (after write): {manifest.get('manifest_sha256_placeholder', 'recorded in manifest file hash externally')}"])
    return "\n".join(lines).rstrip() + "\n"
...
def publish_outputs(records: list[dict[str, Any]], docs: list[dict[str, Any]], excluded: list[dict[str, Any]], stats: dict[str, Any], build_stats: dict[str, Any], template_threshold: int, generic_title_threshold: int) -> dict[str, Any]:
    ...
    backup = RAG_ROOT / "02-retrieval-documents" / ".output.previous"
...
def main() -> int:
    ...
    try:
        ...
        records_by_idx = {int(r["repository_index"]): r for r in records}; fragments_by_repo: dict[int, list[EvidenceFragment]] = {}; excluded_all: list[dict[str, Any]] = []; build_stats = Counter({"source_blocks_total": source_blocks_total, "template_blocks_suppressed": 0, "tiny_generic_blocks_suppressed": 0, "retained_blocks": 0})
        ...
        print("[8/10] Validate document integrity, provenance, and manifest-derived coverage ... ", end="", flush=True)
