...
<portfolio-root>/.dev.vars [PINECONE_API_KEY]
...
OUTPUT
------
rag-corpus/pinecone-v1/dense-parity-validation-v2.json
"""
...
# Storage-fidelity tolerances are intentionally strict because these checks
# compare fetched stored values, not ANN search scores.
...
def build_query_embedding() -> np.ndarray:
    ...
    if native.shape != (1, NATIVE_DIMENSION):
        raise RuntimeError(
            f"Native query shape {native.shape} != (1, {NATIVE_DIMENSION})."
        )
...
def overlap(local: list[str], remote: list[str], k: int) -> dict[str, Any]:
    ...
    return {
        ...
        "shared": len(shared),
        "ratio": len(shared) / k,
        "only_local": sorted(a - b),
        "only_pinecone": sorted(b - a),
    }
...
def main() -> int:
    ...
    if len(sys.argv) != 1:
        print("[1/9] Zero-argument invocation ........ FAILED")
        return 2
    ...
    try:
        ...
        if embeddings.shape != (EXPECTED_COUNT, STORED_DIMENSION):
            raise RuntimeError(f"Unexpected matrix shape {embeddings.shape}.")
        if embeddings.dtype != np.float32:
            raise RuntimeError(f"Unexpected dtype {embeddings.dtype}.")
        if not np.isfinite(embeddings).all():
            raise RuntimeError("Embedding matrix contains NaN/Inf.")
        if len(records) != EXPECTED_COUNT or len(documents) != EXPECTED_COUNT:
            raise RuntimeError("Corpus counts do not equal 2,808.")
        ...
        for rec in records:
            ...
            idx = rec["vector_index"]

            if doc_id not in docs_by_id:
                raise RuntimeError(f"Unknown document ID {doc_id}.")
            if idx in index_to_id:
                raise RuntimeError(f"Duplicate vector_index {idx}.")
            if doc_id in id_to_index:
                raise RuntimeError(f"Duplicate embedding ID {doc_id}.")

            if rec.get("embedding_model") != MODEL_NAME:
                raise RuntimeError(f"Wrong embedding model on {doc_id}.")
            if rec.get("embedding_model_revision") != MODEL_REVISION:
                raise RuntimeError(f"Wrong embedding revision on {doc_id}.")
            if int(rec.get("embedding_dimensions")) != STORED_DIMENSION:
                raise RuntimeError(f"Wrong dimension metadata on {doc_id}.")

            index_to_id[int(idx)] = doc_id
            id_to_index[doc_id] = int(idx)

        if set(index_to_id) != set(range(EXPECTED_COUNT)):
            raise RuntimeError("vector_index coverage is incomplete.")

    except Exception as exc:
        print("[3/9] Validate active corpus .............. FAILED")
        print(f"      {exc}")
        return 1

    print("[3/9] Validate active corpus .............. SUCCESS")
    print(f"      Vectors: {EXPECTED_COUNT}")
    print(f"      Dimension: {STORED_DIMENSION}")

    try:
        query = build_query_embedding()
    except Exception as exc:
        print("[4/9] Build query embedding .............. FAILED")
        print(f"      {exc}")
        return 1
    ...
    try:
        ...
        local_results = [
            {
                "document_id": index_to_id[int(idx)],
                "score": float(local_scores[int(idx)]),
                "vector_index": int(idx),
            }
            for idx in local_indices
        ]
    except Exception as exc:
        print("[5/9] Exact local cosine search .......... FAILED")
        print(f"      {exc}")
        return 1
    ...
    try:
        ...
        if not bool(desc.status.ready):
            raise RuntimeError("Pinecone index is not Ready.")
        if int(desc.dimension) != STORED_DIMENSION:
            raise RuntimeError("Pinecone dimension mismatch.")
        if str(desc.metric).lower() != METRIC:
            raise RuntimeError("Pinecone metric mismatch.")
        ...
        if len(remote_results) != TOP_K:
            raise RuntimeError(
                f"Pinecone returned {len(remote_results)} / {TOP_K} results."
            )

        unknown = [
            r["document_id"]
            for r in remote_results
            if r["document_id"] not in docs_by_id
        ]
        if unknown:
            raise RuntimeError(f"Pinecone returned unknown IDs: {unknown[:5]}")

    except Exception as exc:
        print("[6/9] Pinecone ANN search ................ FAILED")
        print(f"      {exc}")
        return 1

    print("[6/9] Pinecone ANN search ................ SUCCESS")

    local_ids = [r["document_id"] for r in local_results]
    remote_ids = [r["document_id"] for r in remote_results]

    o10 = overlap(local_ids, remote_ids, 10)
    o25 = overlap(local_ids, remote_ids, 25)
    o50 = overlap(local_ids, remote_ids, 50)
    ...
    print("[7/9] ANN candidate parity ............... ", end="")
    ann_pass = all(ann_checks.values())
    print("SUCCESS" if ann_pass else "FAILED")
    ...
    try:
        union_ids = sorted(set(local_ids) | set(remote_ids))

        fetched = index.fetch(ids=union_ids, namespace=NAMESPACE)
        fetched_map = fetched_vectors_dict(fetched)

        if set(fetched_map) != set(union_ids):
            missing_remote = sorted(set(union_ids) - set(fetched_map))
            raise RuntimeError(
                f"Pinecone fetch missing {len(missing_remote)} candidate IDs: "
                f"{missing_remote[:5]}"
            )
        ...
        for doc_id in union_ids:
            local_vector = embeddings[id_to_index[doc_id]]
            remote_vector = fetched_values(fetched_map[doc_id])

            if remote_vector.shape != (STORED_DIMENSION,):
                raise RuntimeError(
                    f"Fetched vector {doc_id} shape {remote_vector.shape} is invalid."
                )

            vector_delta = float(np.max(np.abs(local_vector - remote_vector)))
            local_exact_score = float(local_vector @ query)
            fetched_exact_score = float(remote_vector @ query)
            score_delta = abs(local_exact_score - fetched_exact_score)
            ...
            fidelity_rows.append({
                "document_id": doc_id,
                "max_vector_absolute_delta": vector_delta,
                "local_exact_cosine": local_exact_score,
                "fetched_vector_exact_cosine": fetched_exact_score,
                "recomputed_score_absolute_delta": score_delta,
            })

        storage_checks = {
            "max_vector_delta_within_tolerance": (
                max_vector_delta <= MAX_VECTOR_ABS_DELTA
            ),
            "recomputed_cosine_within_tolerance": (
                max_score_delta <= MAX_RECOMPUTED_SCORE_DELTA
            ),
        }
        ...
        print("[8/9] Stored-vector fidelity ............. FAILED")
        print(f"      {exc}")
        return 1

    print(
        "[8/9] Stored-vector fidelity ............. "
        + ("SUCCESS" if storage_pass else "FAILED")
    )
    print(f"      Max vector abs delta: {max_vector_delta:.10g}")
    print(f"      Max recomputed cosine delta: {max_score_delta:.10g}")

    print()
    print("LOCAL EXACT TOP 10")
    print("------------------")
    for rank, row in enumerate(local_results[:DISPLAY_K], 1):
        doc = docs_by_id[row["document_id"]]
        print(
            f"{rank:2d}. {row['document_id']:<20} "
            f"score={row['score']:.8f} "
            f"repo={int(doc.get('repository_index') or 0):03d} "
            f"{doc.get('repository_name') or ''}"
        )

    print()
    print("PINECONE ANN TOP 10")
    print("-------------------")
    for rank, row in enumerate(remote_results[:DISPLAY_K], 1):
        doc = docs_by_id[row["document_id"]]
        print(
            f"{rank:2d}. {row['document_id']:<20} "
            f"ann_score={row['score']:.8f} "
            f"repo={int(doc.get('repository_index') or 0):03d} "
            f"{doc.get('repository_name') or ''}"
        )
    ...
    try:
        ...
        report = {
            ...
            "ann_candidate_parity": {
                "checks": ann_checks,
                "same_top1": top1_same,
                "overlap_at_10": o10,
                "overlap_at_25": o25,
                "overlap_at_50": o50,
                "result": "PASS" if ann_pass else "FAIL",
            },
            "stored_vector_fidelity": {
                "checks": storage_checks,
                "maximum_vector_absolute_delta": max_vector_delta,
                "maximum_recomputed_cosine_delta": max_score_delta,
                "rows": fidelity_rows,
                "result": "PASS" if storage_pass else "FAIL",
            },
            "pinecone_ann_reported_scores_note": (
                "Pinecone ANN scores are retained for diagnostics but are not "
                "required to numerically equal exhaustive NumPy cosine scores."
            ),
            "local_top_50": local_results,
            "pinecone_top_50": remote_results,
        ...
        print("[9/9] Write validation report ............ FAILED")
        print(f"      {exc}")
        return 1
    ...
    if overall_pass:
        ...
        print(
            "Pinecone is approved to replace the exact dense candidate-search "
            "portion of Retrieval v3."
        )
        return 0

    print("PINECONE DENSE BACKEND VALIDATION: FAIL")
    print("Do not integrate Pinecone into Retrieval v3 yet.")
    return 1
