#!/usr/bin/env python3
"""
Portfolio GitHub RAG pipeline — Step 3 v2: FREE LOCAL embedding generation.
...
Place it beside the existing `rag-corpus/` directory and run:
    ...
    python generate-rag-embeddings-v2-local.py
...
The script embeds ONLY each Step 2 record's `embedding_text` field, with the
model-required RAG document prefix added at embedding time:

    search_document: <embedding_text>

The original `embedding_text`, authoritative `text`, and every Step 2 metadata
field are preserved unchanged in the output evidence records.
...
def validate_token_lengths(model: Any, chunks: list[dict[str, Any]]) -> dict[str, Any]:
    token_counts: list[int] = []
    too_long: list[tuple[str, int]] = []

    tokenizer = model.tokenizer
    total = len(chunks)
    # Tokenize in groups to avoid one Python tokenizer call per chunk while not
    ...
    for start in range(0, total, TOKENIZE_BATCH):
        ...
        texts = [model_input(str(c["embedding_text"])) for c in chunks[start:end]]
        try:
            encoded = tokenizer(
                texts,
        ...
        for offset, ids in enumerate(ids_batch):
            ...
            if count > MAX_SEQUENCE_LENGTH:
                too_long.append((str(chunks[start + offset]["chunk_id"]), count))

        # Print bounded progress so a large corpus never appears frozen.
        if end == total or end % 1024 == 0:
            print(f"\r      Token validation: {end:,}/{total:,} chunks", end="", flush=True)
    ...
    if too_long:
        ...
        raise PipelineError(
            f"{len(too_long)} chunk(s) exceed the fixed {MAX_SEQUENCE_LENGTH}-token "
            "model limit. Step 3 refuses to silently truncate evidence. Return to "
            f"Step 2 and split those chunks. First examples: {preview}"
        )
...
def assemble_from_checkpoints(
    ...
    logical_batch_count = math.ceil(len(chunks) / LOGICAL_BATCH_SIZE)
    new_batches = 0
    resumed_batches = 0
    smallest_encode_batch = INITIAL_ENCODE_BATCH_SIZE

    for logical_no, start in enumerate(
        range(0, len(chunks), LOGICAL_BATCH_SIZE), start=1
    ):
        end = min(start + LOGICAL_BATCH_SIZE, len(chunks))
        ...
        if not loaded:
            raw_texts = [str(chunk["embedding_text"]) for chunk in batch_chunks]
...
def build_embedding_records(chunks: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for vector_index, chunk in enumerate(chunks):
        record = dict(chunk)
        raw_embedding_text = str(chunk["embedding_text"])
        record.update(
            {
...
def validate_matrix_and_records(
    ...
    for index, (chunk, record) in enumerate(zip(chunks, records)):
        ...
        if record.get("model_input_sha256") != sha256_text(
            model_input(chunk["embedding_text"])
        ):
            raise PipelineError(f"Prefixed model-input hash mismatch for chunk {cid}.")
...
def write_temp_outputs(
    ...
    if written != len(chunks):
        raise PipelineError(f"Wrote {written} embedding records; expected {len(chunks)}.")
    ...
    for index, row in enumerate(reloaded_records):
        ...
        if row.get("chunk_id") != chunks[index].get("chunk_id"):
            raise PipelineError(
    ...
    report_lines = [
        "Portfolio GitHub RAG pipeline — Step 3 local embedding validation report",
        "",
        "STATUS: PASS",
        "",
        "COST / PRIVACY",
...
def verify_published_output(expected_input_sha256: str, expected_chunk_count: int) -> None:
    ...
    if count != expected_chunk_count:
        raise PipelineError(
            f"Published embedding record count {count} != {expected_chunk_count}."
        )
...
def print_header() -> None:
    print("Portfolio GitHub RAG pipeline — Step 3 v2: FREE LOCAL embedding generation")
    ...
    print("  Evidence retained: text + original embedding_text + all Step 2 metadata")
    print()
    print("OUTPUT")
...
def main() -> int:
    ...
    try:
        ...
        print("[2/11] Locate Step 2 chunk input ... ", end="", flush=True)
        if not INPUT_PATH.is_file():
            raise PipelineError(
                f"Required input not found: {rel(INPUT_PATH)}\n"
                "Run Step 2 successfully before this script."
            )
        input_sha256 = sha256_file(INPUT_PATH)
        print(f"SUCCESS ({INPUT_PATH.stat().st_size:,} bytes)")

        print("[3/11] Load and validate chunk corpus ... ", end="", flush=True)
        chunks = load_jsonl(INPUT_PATH)
        input_stats = validate_chunks(chunks)
        print(
            f"SUCCESS ({input_stats['chunk_count']:,} chunks; "
        ...
        token_stats = validate_token_lengths(model, chunks)
        print(
            "      SUCCESS "
        ...
        checkpoint_stats = prepare_checkpoint(checkpoint_identity(input_sha256, chunks))
        print(
            "SUCCESS "
        ...
        matrix, run_stats = assemble_from_checkpoints(chunks, model, device)
        print(
            "      SUCCESS "
        ...
        print("Existing validated rag-corpus/embeddings/ output, if any, was not replaced")
